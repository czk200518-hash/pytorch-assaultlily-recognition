...
def _load_anime_cascade():
    if not os.path.exists(ANIME_CASCADE_PATH):
        print(f'[检测] 动漫人脸级联模型不存在，正在下载...')
        try:
            import urllib.request
            url = 'https://raw.githubusercontent.com/nagadomi/lbpcascade_animeface/master/lbpcascade_animeface.xml'
            os.makedirs(os.path.dirname(ANIME_CASCADE_PATH), exist_ok=True)
            urllib.request.urlretrieve(url, ANIME_CASCADE_PATH)
            print(f'[检测] 下载完成: {ANIME_CASCADE_PATH}')
        except Exception:
            print('[检测] 下载失败，使用 OpenCV 内置 Haar 级联')
