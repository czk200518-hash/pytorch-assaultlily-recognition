...
包含:
    - 注意力模块: SEBlock, CBAM
    - 投影头: ProjectionHead (对比学习用)
    - 自定义CNN模型: Tiny, Small, Standard, Large, MobileNet
    - 带注意力的CNN模型: AnimeFaceCNNWithAttention
    - 预训练模型支持: ResNet, EfficientNet
    - 模型工厂函数: create_model
"""
...
class AnimeFaceMobileNet(nn.Module):
    """MobileNetV2 风格 — CPU高效, 深度可分离卷积, ~2M参数"""
...
class AnimeFaceCNNWithAttention(nn.Module):
    ...
    def __init__(self, num_classes: int, input_size: int = 128, attention_type: str = 'cbam', use_contrastive: bool = False):
        ...
        if use_contrastive:
            self.projection_head = ProjectionHead(256, 256, 128)
...
def create_model(model_name: str, num_classes: int, input_size: int = 128, pretrained: bool = True, 
    ...
    可选模型:
        tiny          — 超轻量, ~500K参数, <50张总图
        small         — 轻量, ~3M参数, <100张/类
        standard      — 标准, ~10M参数, 通用
        large         — 大型, ~20M参数, >2000张总图, 最高精度
        mobilenet     — MobileNetV2风格, ~2M参数, CPU高效, 深度可分离卷积
        
        resnet18      — ResNet18, ~11M参数, 迁移学习推荐
        resnet34      — ResNet34, ~21M参数
        resnet50      — ResNet50, ~25M参数, 更强特征提取
        efficientnet_b0 — EfficientNet-B0, ~5M参数, 高效
        efficientnet_b1 — EfficientNet-B1, ~8M参数
        
        standard_se   — 标准模型 + SE注意力
        standard_cbam — 标准模型 + CBAM注意力 (推荐)
    
    参数:
        attention_type: 'se' 或 'cbam'，仅对自定义CNN模型有效
        use_contrastive: 是否启用对比学习投影头
    """
    if model_name == 'tiny':
